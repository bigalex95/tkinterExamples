...
class MainWindow():
    ...
    def update_image(self):
        ...
        # creating images to virtualcam and canvas
        out_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        ht, wd, cc = out_frame.shape
        ...
        result[yy:yy+ht, xx:xx+wd] = out_frame

        print(result.shape)
        out_frame_rgba = np.zeros(
            (self.height, self.width, 4), np.uint8)
        out_frame_rgba[:, :, :3] = result
        out_frame_rgba[:, :, 3] = 255

        # send to virtual webcam
        if self.sendedImage == "n":
            self.cam.send(out_frame_rgba)
        elif self.sendedImage == "w":
...
if __name__ == "__main__":
    ...
    print(height, width)
    root.geometry(
        "{0}x{1}+0+0".format(width-pad, height-pad))
    alpha = 0.1
    root.wait_visibility(root)
    root.wm_attributes('-alpha', alpha)
    # root.iconify()
    root.withdraw()
    root.bind("Return", withdrawWindow)
